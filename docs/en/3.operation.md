...
**Get all selected nodes** : Use `jm.get_selected_nodes()` to get an array of every currently selected node. This is useful after multi-select operations.

**Multi-select subtree** : Hold `Ctrl` (Windows/Linux) or `Cmd` (macOS) while clicking a node to toggle that node and, by default, all of its descendants. You can also invoke `jm.toggle_subtree_selection(node)` directly. Clicking a selected node again deselects it, and deselecting a parent clears any of its selected children. See the `selection` options section for configuring descendant auto-selection or filter callbacks.
When you toggle-select an individual child, every ancestor between it and the nearest already-selected ancestor is re-selected automatically (preserving a contiguous chain); otherwise only the child remains selected.
