...
**selection.enable_multi_select** : (bool) Whether `Ctrl/Cmd + Click` toggling is enabled (default `true`)

> When enabled, jsMind keeps a multi-selection set. Holding the modifier while clicking a node uses `toggle_subtree_selection` behind the scenes so you can select several nodes without losing the previous selection.

**selection.include_descendants** : (bool) Whether to auto-select descendants when toggling (default `true`)

> Set to `false` to only toggle the clicked node instead of selecting its entire subtree.

**selection.filter** : (function) Predicate for skipping nodes when auto-selecting descendants

> Signature: `function filter(node) { return boolean; }`. Return `false` to skip selecting particular nodes (for example, nodes with `node.data.locked = true`). Children of skipped nodes are still evaluated so that deep nodes can participate if the predicate allows them.
