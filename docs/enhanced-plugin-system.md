...
The Enhanced Plugin System is a new plugin architecture for jsMind that provides:
...
The Enhanced Plugin System solves these issues while maintaining full backward compatibility.
...
class MyPlugin extends EnhancedPlugin {
...
**Parameters:**
- `PluginClass` (typeof EnhancedPlugin): Plugin class
...
#### `jsMind.hasEnhancedPlugin(PluginClass)`
...
**Example:**
```javascript
if (jsMind.hasEnhancedPlugin(MyPlugin)) {
...
## Example: MultilineTextV2 Plugin

See `src/plugins/jsmind.multiline-text-v2.js` for a complete example.
...
```javascript
const plugin = new jsMind.plugin('myPlugin', function(jm, options) {
    // Plugin code here
    jm.myPlugin = {
        // Plugin methods
    };
...
Type definitions are provided in `src/jsmind.enhanced-plugin.d.ts`.
...
See `example/multiline-text-v2-demo.html` for a complete working example.
