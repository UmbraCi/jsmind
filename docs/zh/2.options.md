...
**selection.enable_multi_select** : (bool) 是否启用 `Ctrl/Cmd + 点击` 多选（默认 `false`）

> 启用后，按住修饰键点击节点即可利用 `toggle_subtree_selection` 在不清空现有结果的情况下批量选择。

**selection.include_descendants** : (bool) 多选时是否自动包含所有子节点（默认 `true`）

> 将其设为 `false` 时，只会切换当前节点本身的选中状态。

**selection.filter** : (function) 自定义过滤回调

> 签名为 `function filter(node) { return boolean; }`。当返回 `false` 时，该节点在父节点执行多选时不会被默认选中（例如为 `node.data.locked === true` 的节点）。被过滤的节点仍可被单独点击选择，其后代节点也会继续参与判断。
