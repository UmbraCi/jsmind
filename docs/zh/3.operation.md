...
**获取所有选中节点** : 使用 `jm.get_selected_nodes()` 可一次性获取当前被选中的全部节点（支持多选场景）。

**多选子树** : 在节点上按住 `Ctrl`（Windows/Linux）或 `Cmd`（macOS）再点击，可切换该节点及其（默认）所有子节点的选中状态。也可以直接调用 `jm.toggle_subtree_selection(node)`。再次点击已选中的节点会取消选中，父节点取消选中时会同步取消其所有子节点。可通过 `selection` 选项配置是否联动子节点或提供过滤回调。
当你在多选模式下单独选择某个子节点时，只要更高层的祖先已处于选中状态，位于其间的所有父节点都会被自动选中；否则仅保留当前子节点的选中状态，以保持选区连续。
